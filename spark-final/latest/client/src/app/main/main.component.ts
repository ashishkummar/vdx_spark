...
export class MainComponent implements OnInit, AfterViewInit {
  ...
  selected(unitData:any){
    ...
    this.tabIndex=0; // to reset the tab to first.
    ...
    let ht = Number(this.data.size.split('x')[1]);
    let wt = Number(this.data.size.split('x')[0])+100;
    this.unitCss.w = Number(this.data.size.split('x')[0]);
    this.unitCss.h = Number(this.data.size.split('x')[1]);
    ...
    switch(this.data.type.toLowerCase().replace(/\s/g, '')){
      ...
      case "mobileexpandable":
        this.videoCss.width = 414;
        this.videoCss.height = 243;
        this.videoCss.left = 0;
        this.videoCss.top = 65;
        this.unitCss.w = 414;
        this.unitCss.h = 635;
        ht = 635;
        wt = 514;
